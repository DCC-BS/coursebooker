...
export class SQLiteService implements IDatabaseService {
    ...
    // Session operations
    async getSessionsByCourseId(courseId: string): Promise<CourseSession[]> {
        await this.initialize();
        ...
        const sessions: CourseSession[] = [];
        for (const sessionRow of sessionRows) {
            const lessons = await this.getLessonsBySessionId(sessionRow.id);
            sessions.push({
                id: sessionRow.id,
                location: sessionRow.location,
                teams_link: sessionRow.teams_link,
    ...
    async createSession(
        ...
        const transaction = this.db.transaction(
            (courseId: string, session: CourseSession) => {
                // Insert session
                this.db
                    .prepare(`
                    INSERT INTO sessions (id, course_id, location, teams_link)
        ...
        const transaction = this.db.transaction(
            (sessionId: string, updates: Partial<CourseSession>) => {
                ...
                if (updates.location !== undefined) {
                    sessionFields.push("location = ?");
                    sessionParams.push(updates.location);
                }
                if (updates.teams_link !== undefined) {
                    sessionFields.push("teams_link = ?");
                    sessionParams.push(updates.teams_link);
                }

                if (sessionFields.length > 0) {
                    sessionFields.push("updated_at = CURRENT_TIMESTAMP");
                    sessionParams.push(sessionId);

                    this.db
                        .prepare(
                            `UPDATE sessions SET ${sessionFields.join(", ")} WHERE id = ?`,
                        )
                        .run(...sessionParams);
                }
                ...
                if (updates.lessons !== undefined) {
                    ...
                    this.db
                        .prepare("DELETE FROM lessons WHERE session_id = ?")
                        .run(sessionId);
    ...
    // Lesson operations
    async getLessonsBySessionId(sessionId: string): Promise<Lesson[]> {
        await this.initialize();
    ...
    async createLesson(sessionId: string, lesson: Lesson): Promise<Lesson> {
        await this.initialize();
    ...
    async updateLesson(
        ...
        if (lessonFields.length > 0) {
            lessonFields.push("updated_at = CURRENT_TIMESTAMP");
            lessonParams.push(lessonId);

            this.db
                .prepare(
                    `UPDATE lessons SET ${lessonFields.join(", ")} WHERE id = ?`,
                )
                .run(...lessonParams);
        }
